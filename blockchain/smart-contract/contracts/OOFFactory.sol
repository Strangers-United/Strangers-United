...
contract OOFFactory {
    ...
    constructor(address _oofImplementation, address payable _conjureRouter) {
        require(
            _oofImplementation != address(0),
            "No zero address for _oofImplementation"
        );
        require(
            _conjureRouter != address(0),
    ...
    function newOOFImplementation(address oofImplementation_) external {
        ...
        require(
            oofImplementation_ != address(0),
            "No zero address for oofImplementation_"
        );
    ...
    function newConjureRouter(address payable conjureRouter_) external {
        require(msg.sender == factoryOwner, "Only factory owner");
        require(
            conjureRouter_ != address(0),
            "No zero address for conjureRouter_"
        );
