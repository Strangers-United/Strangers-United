...
contract OOFTWAP {
    ...
    function getTWAP(
        IOpenOracleFramework OOFContract,
        uint256[] memory feedIDs,
        ...
        require(
            feedIDs.length == timestampstart.length &&
                feedIDs.length == timestampfinish.length,
            "Feeds and Timestamps must match"
        );

        (, , timeslot, , ) = OOFContract.getFeedList(feedIDs);

        for (uint256 c = 0; c < feedLen; c++) {
            uint256 twapCount = timestampfinish[c] /
                timeslot[c] -
                timestampstart[c] /
                timeslot[c] +
                1;
            ...
            if (strictMode) {
                require(
                    totals[0] != 0 && totals[totals.length - 1] != 0,
                    "Strict Mode: no 0 values for first and last element"
                );
            }
