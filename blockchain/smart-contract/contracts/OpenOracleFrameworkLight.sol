...
contract OpenOracleFrameworkLight {
    ...
    mapping(uint256 => mapping(address => feedSignerStruct))
        private feedToStructMapping;
        ...
        onlySigner
    {
        ...
        for (uint256 i = 0; i < values.length; i++) {
            ...
            if (MAXIMUM_DECIMALS != feedList[feedIDs[i]].feedDecimals) {
                values[i] =
                    values[i] *
                    10**(MAXIMUM_DECIMALS - feedList[feedIDs[i]].feedDecimals);
            }
            ...
            for (uint256 j = 0; j < signers.length; j++) {
                if (
                    (feedToStructMapping[feedIDs[i]][signers[j]].timestamp /
                        feedList[feedIDs[i]].feedTimeslot) ==
                    (block.timestamp / feedList[feedIDs[i]].feedTimeslot)
                ) {
                    signedFeedsLen++;
                    prices[k++] = feedToStructMapping[feedIDs[i]][signers[j]]
                        .value;
                }
            }
